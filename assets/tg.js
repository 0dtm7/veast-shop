...
function saveOrderHistory(next) { localStorage.setItem(ORDER_HISTORY_KEY, JSON.stringify(next.slice(-50))); }
...
function getTelegramRawDataCandidates() {
  ...
  [hash, search].filter(Boolean).forEach((raw) => {
    candidates.push(raw);
    try {
...
function detectSystemTheme() {
  return (tg?.colorScheme === 'light' || tg?.colorScheme === 'dark')
    ? tg.colorScheme
...
function setTheme(theme, persist = true) {
  ...
  updateThemeButton();
  try {
    tg?.setHeaderColor?.(state.theme === 'light' ? '#f2f7ff' : '#08111d');
    tg?.setBackgroundColor?.(state.theme === 'light' ? '#f2f7ff' : '#08111d');
  } catch {}
...
function updateCounters() {
  ...
  $$('[data-cart-count-small]').forEach((n) => { n.textContent = count ? `· ${count}` : ''; });
}

function sheetIsOpen() { return $('[data-product-sheet]')?.classList.contains('is-open'); }
function lightboxIsOpen() { return $('[data-lightbox]')?.classList.contains('is-open'); }
function setView(view) {
  ...
  if (view === 'profile') { renderProfile(); syncOrdersFromServer(); }
...
function renderTelegramButtons() {
  if (!tg) return;
  const canGoBack = sheetIsOpen() || lightboxIsOpen() || !['drop'].includes(state.view);
...
function renderCategories() {
  ...
  node.innerHTML = categories.map((c) => `<button class="tg-chip ${state.activeCategory === c.id ? 'active' : ''}" type="button" data-category="${escapeHtml(c.id)}">${escapeHtml(c.title)}</button>`).join('');
}
...
function productImages(product) {
  return [product.image, product.imageAlt, ...(product.gallery || []), product.cardImage, product.cardImageAlt]
    .filter(Boolean)
...
function productSheet(product) {
  ...
  const current = state.selectedImage || images[0] || product.image || product.cardImage || '';
  ...
  const thumbs = images.map((image) => `<button class="tg-gallery-thumb ${image === current ? 'active' : ''}" type="button" data-sheet-image="${escapeHtml(image)}"><img src="${escapeHtml(image)}" alt="${escapeHtml(product.title)}" /></button>`).join('');
  return `
    <button class="tg-sheet-close" type="button" data-close-sheet aria-label="Закрыть карточку">×</button>
    <div class="tg-sheet-media-wrap">
      <img class="tg-sheet-image" src="${escapeHtml(current)}" alt="${escapeHtml(product.title)}" data-open-lightbox="${escapeHtml(current)}" />
      <button class="tg-photo-open" type="button" data-open-lightbox="${escapeHtml(current)}">посмотреть фото</button>
    </div>
    <div class="tg-gallery-thumbs">${thumbs}</div>
    <div class="tg-sheet-body">
      ...
      <div class="tg-premium-price"><span>Цена</span><strong>${money(product.price)}</strong></div>
...
function openProduct(productId) {
  ...
  state.selectedImage = productImages(product)[0] || product.image || product.cardImage || '';
  const sheet = $('[data-product-sheet]');
  $('[data-sheet-panel]').innerHTML = productSheet(product);
...
function selectSheetImage(image) {
...
function closeLightbox() {
  ...
  if (!sheetIsOpen()) document.body.classList.remove('modal-open');
  renderTelegramButtons();
}
function addToCart(productId, size = '') {
  const product = productById(productId); if (!product) return;
  const selectedSize = size || product.sizes?.[0] || 'OS'; const lineId = `${product.id}-${selectedSize}`; const next = cart(); const existing = next.find((item) => item.lineId === lineId);
  if (existing) existing.quantity += 1; else next.push({ lineId, productId: product.id, size: selectedSize, quantity: 1 });
  saveCart(next); toast(`${product.title} добавлен в корзину`); tg?.HapticFeedback?.notificationOccurred?.('success');
}
...
function renderCart() {
  const list = $('[data-cart-list]'); const summary = $('[data-cart-summary]'); if (!list || !summary) return; const items = buildCartItems();
  ...
  summary.innerHTML = `<div class="tg-total-row"><span>Позиций</span><strong>${items.reduce((sum, item) => sum + item.quantity, 0)}</strong></div><div class="tg-total-row"><span>Товары</span><strong>${money(cartTotal())}</strong></div><div class="tg-total-row"><span>Оплата</span><strong>после подтверждения</strong></div><button class="tg-button tg-button-primary full" type="button" data-view="checkout">🛒 Оформить заказ</button>`;
  renderTelegramButtons();
}
function prefillCheckout() {
  const form = $('[data-checkout-form]'); if (!form) return; const user = getTelegramUser();
  if (user) { const name = [user.firstName, user.lastName].filter(Boolean).join(' ').trim(); if (name && !form.elements.name.value) form.elements.name.value = name; if (user.username && !form.elements.phone.value) form.elements.phone.value = `@${user.username}`; }
...
function buildOrder(form) {
  ...
  const commentParts = [String(form.elements.comment.value || '').trim(), user?.username ? `Telegram: @${user.username}` : '', user?.id ? `Telegram ID: ${user.id}` : ''].filter(Boolean);
  return { customer: { name: form.elements.name.value, phone: form.elements.phone.value, email, city: form.elements.city.value, address: form.elements.address.value, delivery: form.elements.delivery.value, payment: 'После подтверждения', comment: commentParts.join('\n'), privacyAccepted: Boolean(form.elements.privacy.checked) }, items: buildCartItems(), total: cartTotal(), telegram: user };
}
function normalizeOrderForHistory(order) {
  return { id: order.id, total: order.total, items: order.items || [], statusKey: order.statusKey || 'created', status: order.status || 'Заказ создан', statusText: order.statusText || 'Принят', createdAt: order.createdAt || order.created_at || new Date().toISOString() };
}
function rememberOrder(order) {
  const publicOrder = normalizeOrderForHistory(order || {});
  localStorage.setItem(LAST_ORDER_KEY, JSON.stringify(publicOrder));
  const history = orderHistory().filter((item) => item.id !== publicOrder.id);
  history.push(publicOrder);
  saveOrderHistory(history);
  state.activeOrderId = publicOrder.id;
}
async function submitOrder(event) {
  event.preventDefault(); const form = event.currentTarget;
  if (!cart().length) { toast('Сначала добавь товар в корзину'); setView('catalog'); return; }
  const button = form.querySelector('button[type="submit"]'); const original = button.textContent; button.disabled = true; button.textContent = 'Отправляем...';
  try {
    const response = await fetch('/api/orders', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(buildOrder(form)) });
    const data = await response.json().catch(() => ({}));
    if (!response.ok || !data.ok) throw new Error(data.error || `HTTP ${response.status}`);
    rememberOrder(data.order || {}); saveCart([]); await syncOrdersFromServer(true);
    $('[data-success-text]').textContent = `Заказ ${data.order?.id || ''} создан. VEAST свяжется с тобой для подтверждения.`; renderProfile(); setView('success'); tg?.HapticFeedback?.notificationOccurred?.('success');
  } catch (error) { toast(error.message || 'Не удалось отправить заказ'); tg?.HapticFeedback?.notificationOccurred?.('error'); }
  finally { button.disabled = false; button.textContent = original; }
}

async function syncOrdersFromServer(force = false) {
  const user = getTelegramUser();
  if (!user?.id) return;
  if (state.ordersSynced && !force) return;
  state.ordersSynced = true;
  try {
    const response = await fetch(`/api/telegram/orders?chatId=${encodeURIComponent(user.id)}`, { headers: { Accept: 'application/json' } });
    const data = await response.json().catch(() => ({}));
    if (!response.ok || !data.ok || !Array.isArray(data.orders)) return;
    const merged = new Map();
    orderHistory().forEach((order) => { if (order?.id) merged.set(order.id, order); });
    data.orders.forEach((order) => { if (order?.id) merged.set(order.id, normalizeOrderForHistory(order)); });
    const next = [...merged.values()].sort((a, b) => new Date(a.createdAt || 0) - new Date(b.createdAt || 0));
    saveOrderHistory(next);
    renderProfile();
    if (state.view === 'order') renderOrderDetail();
  } catch {}
}
...
async function refreshOrderStatus(id = state.activeOrderId) {
  if (!id) return;
  try {
    const response = await fetch(`/api/orders/${encodeURIComponent(id)}/status`);
    if (!response.ok) return;
    const status = await response.json();
    const history = orderHistory(); const index = history.findIndex((order) => order.id === id);
    if (index >= 0) { history[index] = { ...history[index], ...status, statusKey: status.statusKey || history[index].statusKey, statusText: status.statusText || history[index].statusText }; saveOrderHistory(history); renderProfile(); renderOrderDetail(history[index]); }
  } catch {}
}
function openLastOrder() { const last = read(LAST_ORDER_KEY, null); if (last?.id) openOrder(last.id); else setView('profile'); }

async function loadProducts() {
  $('[data-products]').innerHTML = loadingSkeleton(); $('[data-featured-products]').innerHTML = loadingSkeleton();
  try {
    const response = await fetch('/api/products', { headers: { Accept: 'application/json' } });
    const data = await response.json();
    state.products = Array.isArray(data.products) ? data.products : [];
    state.categories = Array.isArray(data.categories) ? data.categories : [{ id: 'all', title: 'Все товары' }];
    renderAll(); await syncOrdersFromServer();
    const start = getStartParam(); if (start) { const product = productById(start.replace(/^product_/, '')); if (product) setTimeout(() => openProduct(product.id), 250); }
  } catch (error) { $('[data-products]').innerHTML = `<div class="tg-empty"><strong>каталог не загрузился</strong><span>${escapeHtml(error.message)}</span></div>`; toast('Каталог временно не загрузился'); }
}
...
function bindEvents() {
  document.addEventListener('click', (event) => {
    const target = event.target.closest('button, [data-open-product], [data-open-lightbox]'); if (!target) return;
    ...
    if (target.dataset.category) { state.activeCategory = target.dataset.category; renderCategories(); renderProducts(); }
    if (target.dataset.openProduct) openProduct(target.dataset.openProduct);
    if (target.dataset.closeSheet !== undefined) closeProduct();
    if (target.dataset.closeLightbox !== undefined) closeLightbox();
    if (target.dataset.sheetImage) selectSheetImage(target.dataset.sheetImage);
    if (target.dataset.openLightbox) openLightbox(target.dataset.openLightbox);
    if (target.dataset.selectSize) { state.selectedSize = target.dataset.selectSize; if (state.selectedProduct) $('[data-sheet-panel]').innerHTML = productSheet(state.selectedProduct); }
    if (target.dataset.addSelected) { addToCart(target.dataset.addSelected, state.selectedSize); closeProduct(); }
    ...
    if (target.dataset.openLastOrder !== undefined) openLastOrder();
    if (target.dataset.refreshOrder) refreshOrderStatus(target.dataset.refreshOrder);
    if (target.dataset.syncOrders !== undefined) { state.ordersSynced = false; syncOrdersFromServer(true); toast('Обновляю заказы'); }
  });
  $('[data-lightbox]')?.addEventListener('click', (event) => { if (event.target.matches('[data-lightbox]')) closeLightbox(); });
  document.addEventListener('keydown', (event) => { if (event.key === 'Escape') { if (lightboxIsOpen()) closeLightbox(); else if (sheetIsOpen()) closeProduct(); } });
  $('[data-search]')?.addEventListener('input', (event) => { state.query = event.currentTarget.value; renderProducts(); });
  $('[data-checkout-form]')?.addEventListener('submit', submitOrder);
  tg?.BackButton?.onClick(() => { if (lightboxIsOpen()) closeLightbox(); else if (sheetIsOpen()) closeProduct(); else if (state.view === 'checkout') setView('cart'); else if (state.view === 'order') setView('profile'); else if (state.view !== 'drop') setView('drop'); });
